...
public class AccountModel extends AccountDao {
  ...
  public List<Account> findByOwner(Object owner) {
    if(owner  == null)
      return null;
    EntityManager em = getEntityManager();
    try {
      return em.createQuery("SELECT obj FROM Account obj WHERE obj.owner.id = :id")
        .setParameter("id", (owner instanceof String ? (String)owner : ((User)owner).getId()))
        .getResultList();
    } catch (Exception e) {
      System.out.print("An error occurred while getting owner = '" + 
        (owner instanceof String ? (String)owner : ((User)owner).getId()) + "' from table Account.\n\n Error:" + e + "\n\n");
      return null;
    } finally {
      em.close();
    }
